...
4. **Adjust Script Order**
   - Edit the `SCRIPTS` array in `bootstrap.sh` to change execution order
...
1. **Scripts Not Found**
   - Ensure you're running `bootstrap.sh` from the bootstrap directory
...
3. **Permission Issues**
   ...
   - Run `chmod +x bootstrap.sh scripts/*.sh` if needed
